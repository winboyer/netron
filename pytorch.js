...
pytorch.Graph = class {
    ...
    _loadModule(metadata, parent, groups, inputs) {
        ...
        const sequential = parent.__module__ && parent.__module__ === 'torch.nn.modules.container' && parent.__name__ === 'Sequential';
            ...
            if (module && value) {
                ...
                switch (type) {
                    ...
                    case 'torchvision.models.inception.InceptionE': {
                        groups.push(key);
                        const node = this._createNode(metadata, groups, key, value, sequential ? inputs : [], this._littleEndian);
                        inputs = [ node.name ];
                        groups.pop(key);
                        break;
                    }
                    default: {
                        const node = this._createNode(metadata, groups, key, value, sequential ? inputs : []);
                        inputs = [ node.name ];
                        break;
                    }
                }
